...
## 文件类型
冯 诺伊曼曰：计算器、控制器、主存储器、输入输出设备。类Unix系统做了一个抽象：一切皆文件。基于文件就可以应用到IO流。在Linux中文件的类型其实有
...
```
这里可以看到，这个的偏移量位0，多个bash程序打开同一个文件，但是每个fd各自维护各自的指针。有点像Java中的迭代器
现在开始读几行文件：
```
...
cd /proc/$$/fd
```
虚拟文件系统，他的每个目录并不一定都在磁盘上，像是proc，只有开机之后/proc才有内容，下面的每个数字就是pid，这又一次说明了"一切皆文件"。$$
...
```
ls命令的输出，一个走了1一个走了2，重定向标准输出，就只剩下报错信息了。
```
...
[root@localhost ~]#
```
这就是为什么/etc/profile设置环境变量的时候需要export他们，父进程定义好了，各个进程就都知道有这些变量了。
...
表里面会有0、1、2...128、... 255号（一个字节），128这里有一个callback （方法）在用户程序里面，想调用某个系统调用的时候，其实是完成了一个
...
了，可以忙点别的。这样比什么事情都停了、CPU一直在搬数据这个过程更高效。
