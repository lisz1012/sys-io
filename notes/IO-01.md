...
```
其中`/boot`虽然看上去像是跟其他目录是在一起的，他其实的物理位置是`/dev/sda1`,覆盖了sda3。根目录下其他的东西都在`/dev/sda3 `
...
Java 流的write方法不flush的话就写入了pagecache，此时计算机如果断电，已经写完了的数据就会丢失
...
VFS树、inode、进程间隔离、pagecache在内存里， 4k、dirty、flush  